(() => {
    ...
    function updateFlight(dt,now) {
      const screenForward=Math.max(-1,Math.min(1,(keys.up?1:0)-(keys.down?1:0)-touchAxis.y));
      const screenRight=Math.max(-1,Math.min(1,(keys.right?1:0)-(keys.left?1:0)+(keys.e?1:0)-(keys.q?1:0)+touchAxis.x));
      const cameraVertical=(keys.w?1:0)-(keys.s?1:0);
      const cameraHorizontal=(keys.d?1:0)-(keys.a?1:0);
      const manualMagnitude=Math.min(1,Math.hypot(screenForward,screenRight));
      const manualMove=manualMagnitude>.025;
      ...
      if (moving) {
        ...
        } else targetSpeed=78*manualMagnitude*boost;
      }
      state.speed += (targetSpeed-state.speed)*Math.min(1,dt*4);
      const moveSpeed=moving?(state.autopilot?Math.max(16,state.speed):Math.max(4,state.speed)):state.speed;
      if (moving) {
    ...
    const joystick=$('moveJoystick'), joystickKnob=$('joystickKnob');
    let joystickPointer=null;
    function updateJoystick(e) {
      const rect=joystick.getBoundingClientRect();
      const scaleX=joystick.clientWidth/Math.max(1,rect.width), scaleY=joystick.clientHeight/Math.max(1,rect.height);
      const dx=(e.clientX-(rect.left+rect.width/2))*scaleX;
      const dy=(e.clientY-(rect.top+rect.height/2))*scaleY;
      const distance=Math.hypot(dx,dy);
      const maxRadius=Math.max(24,(Math.min(joystick.clientWidth,joystick.clientHeight)-joystickKnob.offsetWidth)/2-7);
      const limited=Math.min(distance,maxRadius);
      const ux=distance?dx/distance:0, uy=distance?dy/distance:0;
      const strength=Math.max(0,(limited/maxRadius-.14)/.86);
      touchAxis.x=ux*strength; touchAxis.y=uy*strength;
      joystickKnob.style.transform='translate('+(ux*limited).toFixed(1)+'px,'+(uy*limited).toFixed(1)+'px)';
    }
    function resetJoystick(e) {
      if(e&&joystickPointer!==null&&e.pointerId!==undefined&&e.pointerId!==joystickPointer)return;
      joystickPointer=null; touchAxis.x=0; touchAxis.y=0;
      joystick.classList.remove('active'); joystickKnob.style.transform='translate(0,0)';
    }
    joystick.addEventListener('pointerdown',e=>{
      if(joystickPointer!==null||e.button>0)return;
    ...
    joystick.addEventListener('pointermove',e=>{if(e.pointerId===joystickPointer){e.preventDefault();updateJoystick(e);}});
    joystick.addEventListener('pointerup',resetJoystick);
    joystick.addEventListener('pointercancel',resetJoystick);
    joystick.addEventListener('lostpointercapture',resetJoystick);
    ['selectstart','contextmenu','dragstart'].forEach(type=>$('touchControls').addEventListener(type,e=>e.preventDefault()));
    ...
    function setMobileMenu(open) {
      if(mobileLayout) $('flightMenu').hidden=!open;
      else $('flightMenu').hidden=false;
